...
//============================================================== inner class listeners
    ...
    public class MySendListener implements ActionListener {    // new - save
        public void actionPerformed(ActionEvent a) { //Все это прроисходит при нажатии кнопки, после чего срабатывает ActionEvent

            boolean[] checkboxState = new boolean[256]; //Создаем булев массив для хранения состояния каждого флажка

            for (int i = 0; i < 256; i++) {
                JCheckBox check = (JCheckBox) checkboxList.get(i); //пробегаем через checkboxList(ArrayList, содержащий состояния флажков),
...
//Записываем/ сериализуем булев массив
    ...
    public class MyReadInListener implements ActionListener {  // new - restore
        public void actionPerformed(ActionEvent a) {
            ...
            try {// Считываем объект из файла и определяем его как булев массив(readObject() возвращает ссылку на тип Object())
            ...
            // теперь восстанавливаем состояние каждого флажка в  ArrayList содержащий обьекты  JCheckBox(checkboxList)
            for (int i = 0; i < 256; i++) {
            ...
            // Здесь мы останавливаем проигрывание мелодии и восстанавливаем последовательность, используя новые состояния флажков в ArrayList
            sequencer.stop();
            buildTrackAndStart();
        } // close method
    } // close inner class
